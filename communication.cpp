...
void setupWiFi() {
    ...
    // ===== WiFi 스캔 (실제 SSID 확인용) =====
    Serial.println("[SCAN] Scanning available networks...");
    int numSsid = WiFi.scanNetworks();
    if (numSsid == -1) {
        Serial.println("[WARN] Scan failed, but continuing...");
    } else {
    ...
    while (WiFi.status() != WL_CONNECTED) {
        ...
        if (retry > 30) {
            ...
            Serial.print("[STATUS] WiFi.status() = ");
...
long extractCommandId(String json) {
    int idx = json.indexOf("\"commandId\":");
    int prefixLen = 12;
    
    if (idx < 0) {
        idx = json.indexOf("\"command_id\":");
        prefixLen = 13;
        if (idx < 0) return 0;
    }
